...
class Parser():
    ...
    async def get_areas():
        ...
        options = galuzi.find_all('option')
        ...
        await Parser.get_specs(links_of_areas)
        print(f'=======FINISHED=======\n{time()-start_time} ')

    @staticmethod
    async def courses_ln_gen(courses):
    ...
    @staticmethod
    async def get_specs(links_of_areas):
        ...
        for link in links_of_areas:
            ...
            area_courses = {'area': link['code'],
                            'links': await Parser.courses_ln_gen(courses)}
            ...
            for course in area_courses['links']:
                ...
                # get coefficients
                coefficients = await Parser.get_coefficients(soup)
                try:
    ...
    @staticmethod
    async def get_coefficients(soup):
        ...
        # find tag with variants of 3rd subject
