...
class Parser(DatabaseMapper):
    ...
    def get_specs(links_of_areas):
        ...
        for link in links_of_areas:
            ...
            area_courses = {'area':link['code'],'links':Parser.courses_ln_gen(courses)}
        ...
        for course in area_courses['links']:
            ...
            spec['name'] = soup.find('div',class_='page-vnz-detail-title').find('h1').findAll('b')[1].text
            spec['program'] = soup.find('div',class_='page-vnz-detail-title').find('h1').findAll('b')[0].text
            coefficients = Parser.get_coefficients(soup)
            print(coefficients)
            try:
                linkfordetails = soup.findAll('table',class_="stats-vnz-table")[0].find('a')
            except: continue
    ...
    def get_coefficients(soup):
        ...
        znos.append({'name':first.find('b').text,'coefficient':first.find('span',class_='coef').text,'required':True})
        second = soup.find('li',class_='subject_2')
        znos.append({'name':second.find('b').text,'coefficient':second.find('span',class_='coef').text,'required':True})
        ...
        for subject in third:
            znos.append({'name':subject.find('b').text,
                        'coefficient':subject.find('span',class_='coef').text,'required':False})

        attestat = soup.find('li',class_='subject_100')
        znos.append({'name':attestat.find('b').text,'coefficient':attestat.find('span',class_='coef').text,'required':True})
